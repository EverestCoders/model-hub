...
contract ModelMarketplace {
    ...
    function getUserModelIds(
        string memory _userId
    ) public view returns (uint256[] memory) {
        return userModels[_userId].modelIds;
    }

    function verifyModelStorage(
        uint256 _modelId
        ...
        (
            int256 exit_code,
            MarketTypes.GetDealDataCommitmentReturn memory commitment
    ...
    function purchaseAccess(uint256 _modelId) public payable {
        ...
        ModelInfo storage model = models[_modelId];
        require(
            msg.value == model.accessFee,
            "Payment amount must match access fee"
        );
    ...
    function getModelPaymentsCount(
        uint256 _modelId
    ) public view returns (uint256) {
        return modelPayments[_modelId].length;
    }

    function getModelPaymentAt(
        uint256 _modelId,
        uint256 _index
    )
        public
        ...
        returns (address, address, uint256, uint256, uint256, string memory)
    {
        require(_modelId <= modelCounter && _modelId > 0, "Invalid model ID");
        require(
            _index < modelPayments[_modelId].length,
